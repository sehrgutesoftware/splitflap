...
spool_axle_radius =  flap_pitch_radius - 4 * flap_hole_radius;

spool_bearing_inner_radius = 4/2; // M4
spool_bearing_outer_radius = 10/2; // 10mm diameter
spool_bearing_thickness = thickness;
spool_bearing_inner_bush_radius = spool_bearing_inner_radius + 1.5;
...
front_window_notch_length = 5;
front_window_notch_width = 10;
...
enclosure_length_right = front_forward_offset + 2*spool_bearing_outer_radius + enclosure_horizontal_rear_margin;
...
module enclosure_front_cutouts_2d(tool_diameter=0) {
    ...
    translate([front_window_right_inset, enclosure_height_lower - front_window_lower])
        difference() {
            ...
            translate([(front_window_width-front_window_notch_width)/2, front_window_height - front_window_notch_length])
                square([front_window_notch_width, front_window_notch_length]);
        }
    ...
    // Front right tabs
    translate([0, 0, 0])
        front_lr_tabs_negative();

    translate([enclosure_wall_to_wall_width-thickness, 0, 0])
        front_lr_tabs_negative();
        ...
        for (x = [0, 1]) {
            for (y = [-1, 1]) {
                translate([
                    ...
                    circle(r=front_mounting_hole_dia/2, $fn=30);
                }
            }
        }
    }
...
module front_lr_tabs_2d(left_side) {
    tab_offset = (enclosure_height) / num_front_tabs;
    y_offset = left_side ? enclosure_length : enclosure_length_right;
    for (i = [0 : num_front_tabs-1]) {
        translate([(i+0.5) * tab_offset - front_tab_width/2, y_offset, 0]) {
            square([front_tab_width, thickness], center=false);
        }
    }
}

module front_lr_tabs_negative() {
    tab_offset = (enclosure_height) / num_front_tabs;
    for (i = [0 : num_front_tabs-1]) {
        translate([0, (i+0.5) * tab_offset - front_tab_width/2, 0]) {
            square([thickness + enclosure_tab_clearance, front_tab_width+enclosure_tab_clearance], center=false);
        }
    }
}
        ...
        union() {
            difference() {
                ...
                translate([enclosure_height_upper, enclosure_length_right - front_forward_offset, 0])
                    circle(r=spool_bearing_outer_radius, $fn=30);
...
if (render_3d) {
    ...
    laser_mirror() {
        ...
        projection_renderer(render_index=render_index, render_etch=render_etch, kerf_width=kerf_width, panel_height=panel_height, panel_horizontal=panel_horizontal, panel_vertical=panel_vertical) {
            ...
            // Right Side Panel
            translate([0, enclosure_width+enclosure_length+thickness+2*kerf_width]) {
                enclosure_right();
                laser_etch()
                    enclosure_right_etch();
            }

            front_panel_offset = enclosure_length + kerf_width + enclosure_width + thickness - enclosure_horizontal_inset;
            ...
            translate(front_panel_origin) {
                ...
                // Connectors
                if (enable_connectors) {
                    translate([0, kerf_width+enclosure_length_right+kerf_width+enclosure_length_right+kerf_width]) {
                        connector_bracket();
                ...
                // Spools
                translate([front_window_height - spool_outer_radius - kerf_width - front_window_notch_length, spool_outer_radius+kerf_width]) {
                    flap_spool_complete(motor_shaft=true, magnet_hole=true);
                    if (!render_home_indicator_as_cut)
                        laser_etch()
                            flap_spool_etch();
                    translate([0, 2*spool_outer_radius+kerf_width])
                        mirror([0, 1, 0]) {
                            flap_spool_complete(captive_nut=true);
                            if (!render_home_indicator_as_cut)
                                laser_etch()
