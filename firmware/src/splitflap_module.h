...
// Logging and assertions are useful for debugging, but likely add too much
// time/space overhead to be used when driving many SplitflapModules at once.
#define VERBOSE_LOGGING false
#define ASSERTIONS_ENABLED false

#define FAKE_HOME_SENSOR false

// Must be an integer. This code does not support a fractional steps per
// revolution for motors with non-integral gear ratios
#define STEPS_PER_REVOLUTION (2048)

// This is "rough" because it's integer division; it shouldn't be used for
// movement calculations or the error would accumulate.
#define _ROUGH_STEPS_PER_FLAP (STEPS_PER_REVOLUTION / NUM_FLAPS)

#if HOME_CALIBRATION_ENABLED
// The number of steps in either direction that's acceptable error for the home
// sensor
#define HOME_ERROR_MARGIN_STEPS (_ROUGH_STEPS_PER_FLAP / 4)

// After finding the home position, how long to wait before considering another
// home blip to be an unexpected error
#define UNEXPECTED_HOME_START_BUFFER_STEPS (_ROUGH_STEPS_PER_FLAP * 5)

// When recalibrating the home position, the number of steps to travel searching
// for home before giving up
#define MAX_STEPS_LOOKING_FOR_HOME ((NUM_FLAPS + 2) * _ROUGH_STEPS_PER_FLAP)

#define UNEXPECTED_HOME_START_STEP                                             \
  UNEXPECTED_HOME_START_BUFFER_STEPS // Start of range where a home sensor blip
                                     // is unexpected
#define UNEXPECTED_HOME_END_STEP                                               \
  (STEPS_PER_REVOLUTION -                                                      \
   HOME_ERROR_MARGIN_STEPS) // End of range where a home sensor blip is
                            // unexpected

// Expected home position step plus some margin of error. If we get to this step
// without having seen a home sensor blip, something is wrong and we need to
// recalibrate.
...
private:
  ...
  // Tracks the most recent target flap index. Not used during motion, but
  // needed to recalculate target step if we re-calibrate the home position
...
public:
  SplitflapModule(uint8_t &motor_out, const uint8_t motor_bitshift,
                  uint8_t &sensor_in, const uint8_t sensor_bitmask);
...
SplitflapModule::SplitflapModule(uint8_t &motor_out,
                                 const uint8_t motor_bitshift,
                                 uint8_t &sensor_in,
                                 const uint8_t sensor_bitmask)
    : motor_out(motor_out), motor_bitshift(motor_bitshift),
      sensor_in(sensor_in), sensor_bitmask(sensor_bitmask) {}
...
__attribute__((always_inline)) inline bool SplitflapModule::CheckSensor() {
...
SplitflapModule::SetMotor(uint8_t out) {
  motor_out = (motor_out & ~(0x0F << motor_bitshift)) |
              ((out & 0x0F) << motor_bitshift);
}

__attribute__((always_inline)) inline uint8_t
SplitflapModule::GetFlapFloor(uint16_t step) {
  uint16_t step_without_offset =
      step >= offset_steps ? step - offset_steps
                           : STEPS_PER_REVOLUTION + step - offset_steps;
  return (uint32_t)step_without_offset * NUM_FLAPS / STEPS_PER_REVOLUTION;
}

__attribute__((always_inline)) inline uint16_t
SplitflapModule::GetTargetStepForFlapIndex(uint8_t flap) {
  uint32_t intermediate = (uint32_t)flap * STEPS_PER_REVOLUTION;

  // Round UP when dividing so that the inverse calculation on the result
  // (GetFlapFloor) returns the expected result.
...
__attribute__((always_inline)) inline void
SplitflapModule::GoToTargetFlapIndex() {
  if (state != NORMAL) {
    return;
  }
  uint16_t target_step = GetTargetStepForFlapIndex(target_flap_index);

  uint16_t minimum_stopping_step =
      current_step +
      current_accel_step; // Can't come to a stop until we've used up any
                          // remaining deceleration steps

  if (target_step <= minimum_stopping_step) {
    // Must go around
    delta_steps = STEPS_PER_REVOLUTION - current_step + target_step;
    // NB: delta_steps can be > 1 full revolution in the event we are not yet at
    // current_step, but couldn't stop in time
  } else {
...
__attribute__((always_inline)) inline void
SplitflapModule::GoToFlapIndex(uint8_t index) {
  if (state != NORMAL
#if HOME_CALIBRATION_ENABLED
      && state != LOOK_FOR_HOME
#endif
...
__attribute__((always_inline)) inline uint8_t
SplitflapModule::GetCurrentFlapIndex() {
  return GetFlapFloor(current_step);
}

uint8_t SplitflapModule::GetTargetFlapIndex() { return target_flap_index; }

__attribute__((always_inline)) inline void
SplitflapModule::FindAndRecalibrateHome() {
#if HOME_CALIBRATION_ENABLED
...
__attribute__((always_inline)) inline void SplitflapModule::Update() {
  if (state == PANIC || state == STATE_DISABLED) {
    return;
  }
  ...
  if (delta_time >= current_period) {
    ...
    current_period = pgm_read_word_near(Acceleration::ACCEL_STEP_PERIODS +
                                        current_accel_step);
...
void SplitflapModule::Init() { CheckSensor(); }
...
uint16_t SplitflapModule::GetOffset() { return offset_steps; }
