...
// Optional - uncomment if connecting the output enable pin of the 74HC595 shift
// registers to the ESP32. You can otherwise hard-wire the output enable pins to
// always be enabled. #define OUTPUT_ENABLE_PIN (27)

#define PIN_NUM_MISO 39
#define PIN_NUM_MOSI 32
#define PIN_NUM_CLK 33

// Note: You may need to slow this down to 3MHz if you're using a classic driver
// board; the MIC5842 only officially supports up to 3.3MHz
...
#if !defined(__AVR_ATmega168__) && !defined(__AVR_ATmega328P__) &&             \
    !defined(ARDUINO_ESP8266_WEMOS_D1MINI) && !defined(ESP32)
#error                                                                         \
    "Unknown/unsupported board for SPI mode. ATmega328-based boards (Uno, Duemilanove, Diecimila), ESP8266 and ESP32 are currently supported"
#endif
...
#ifdef __AVR__
// Define placement new so we can initialize SplitflapModules at runtime into a
// static buffer. (see https://arduino.stackexchange.com/a/1499)
...
#ifdef ESP32
void reset_latch(spi_transaction_t *trans) { digitalWrite(LATCH_PIN, LOW); }
...
inline void initialize_modules() {
  for (uint8_t i = 0; i < NUM_MODULES; i++) {
    // Create SplitflapModules in a statically allocated buffer using placement
    // new
#ifdef CHAINLINK
    modules[i] = new (moduleBuffer[i]) SplitflapModule(
        motor_buffer[MOTOR_BUFFER_LENGTH - 1 - i / 6 * 4 - MOTOR_OFFSET[i % 6]],
        i % 2 == 0 ? 0 : 4, sensor_buffer[i / 6], 1 << (i % 6));
#else
    modules[i] = new (moduleBuffer[i])
        SplitflapModule(motor_buffer[MOTOR_BUFFER_LENGTH - 1 - i / 2],
                        i % 2 == 0 ? 0 : 4, sensor_buffer[i / 4], 1 << (i % 4));
#endif
  }
...
void chainlink_set_led(uint8_t moduleIndex, bool on) {
  ...
  uint8_t byteIndex = MOTOR_BUFFER_LENGTH - 1 - moduleIndex / 6 * 4 -
                      (groupPosition < 3 ? 1 : 2);
  uint8_t bitMask = (groupPosition < 3) ? (1 << (4 + groupPosition))
                                        : (1 << (groupPosition - 3));
  if (on) {
...
static uint8_t chainlink_loopbackMotorByte(uint8_t loopbackIndex) {
  return MOTOR_BUFFER_LENGTH - 1 - (loopbackIndex / 2) * 4 -
         (((loopbackIndex % 2) == 0) ? 1 : 2);
}
...
bool chainlink_test_startup_loopback(bool results[NUM_LOOPBACKS]) {
  ...
  // Turn off all motors, leds, and loopbacks; make sure all loopback inputs
  // read 0
  memset(motor_buffer, 0, MOTOR_BUFFER_LENGTH);
  motor_sensor_io();
  motor_sensor_io();
  ...
  for (uint8_t i = 0; i < NUM_LOOPBACKS; i++) {
    results[i] = ((sensor_buffer[chainlink_loopbackSensorByte(i)] &
                   chainlink_loopbackSensorBitMask(i))) == 0;
    success &= results[i];
  }
  return success;
}
...
void chainlink_set_loopback(uint8_t loop_out_index) {
  ...
  motor_buffer[chainlink_loopbackMotorByte(loop_out_index)] |=
      chainlink_loopbackMotorBitMask(loop_out_index);
}

/**
 * Validate that the loopback from loop_out_index can be read successfully.
 * There must be AT LEAST 2 motor_sensor_io() invocations between setting the
 * loopback and validating it - one for turning on the shift register output and
 * another to read in the shift register input.
 */
bool chainlink_validate_loopback(uint8_t loop_out_index,
                                 bool results[NUM_LOOPBACKS]) {
  bool success = true;
  for (uint8_t loop_in_index = 0; loop_in_index < NUM_LOOPBACKS;
       loop_in_index++) {
    uint8_t expected_bit_mask =
        (loop_out_index == loop_in_index)
            ? chainlink_loopbackSensorBitMask(loop_in_index)
            : 0;
    uint8_t actual_bit_mask =
        sensor_buffer[chainlink_loopbackSensorByte(loop_in_index)] &
        chainlink_loopbackSensorBitMask(loop_in_index);
...
bool chainlink_test_all_loopbacks(
    bool loopback_result[NUM_LOOPBACKS][NUM_LOOPBACKS],
    bool loopback_off_result[NUM_LOOPBACKS]) {
  bool loopback_success = true;

  // Turn one loopback bit on at a time and make sure only that loopback bit is
  // set
  for (uint8_t loop_out_index = 0; loop_out_index < NUM_LOOPBACKS;
       loop_out_index++) {
    chainlink_set_loopback(loop_out_index);
    motor_sensor_io();
    motor_sensor_io();
    loopback_success &= chainlink_validate_loopback(
