...
void SerialLegacyJsonProtocol::handleState(const SplitflapState &old_state,
                                           const SplitflapState &new_state) {
...
void SerialLegacyJsonProtocol::loop() {
  ...
  while (stream_.available() > 0) {
    ...
    if (b == '%') {
      bool new_sensor_test_state =
          latest_state_.mode != SplitflapMode::MODE_SENSOR_TEST;
      ...
      case '>':
        ...
        for (int i = 0; i < NUM_FLAPS; i++) {
          splitflap_task_.increaseOffsetTenth(i);
        }
        break;
      case '<':
        // TODO: make the index configurable
        for (int i = 0; i < NUM_FLAPS; i++) {
          splitflap_task_.increaseOffsetHalf(i);
        }
        break;
      case '\\':
...
void SerialLegacyJsonProtocol::sendSupervisorState(
    PB_SupervisorState &supervisor_state) {
  // Intentionally not implemented.
  // Advanced features like supervisor state are not supported via the legacy
  // protocol; use the proto protocol instead.
}
