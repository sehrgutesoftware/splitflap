...
// About this example:
// - Fetches current weather data for an area in San Francisco (updating
// infrequently)
// - Cycles between showing the temperature and the wind speed on the
// split-flaps (cycles frequently)
//
...
// - cycling through messages at a different interval than data is loaded (see
// run)
...
// (exaggerated for example purposes)
...
// Public token for synoptic data api (it's not secret, but please don't abuse
// it)
#define SYNOPTICDATA_TOKEN "e763d68537d9498a90fa808eb9d415d9"

// Timezone for local time strings; this is America/Los_Angeles. See
...
bool HTTPTask::fetchData() {
  ...
  http.begin("https://api.synopticdata.com/v2/stations/"
             "latest?&token=" SYNOPTICDATA_TOKEN
             "&within=30&units=english&vars=air_temp,wind_speed&varsoperator="
             "and&radius=37.765157,-122.419702,4&limit=20&fields=stid");
  ...
  snprintf(buf, sizeof(buf), "Finished request in %lu millis.",
           millis() - start);
  logger_.log(buf);
  if (http_code > 0) {
    ...
    snprintf(buf, sizeof(buf), "Response code: %d Data length: %d", http_code,
             data.length());
    logger_.log(buf);
    ...
    snprintf(buf, sizeof(buf), "Error on HTTP request (%d): %s", http_code,
...
bool HTTPTask::handleData(Json json) {
  // Extract data from the json response. You could use ArduinoJson, but I find
  // json11 to be much easier to use albeit not optimized for a microcontroller.
  ...
  */
  ...
  if ((entries % 2) == 0) {
    ...
    median_wind_speed =
        (wind_speeds[entries / 2 - 1] + wind_speeds[entries / 2]) / 2;
  } else {
  ...
  snprintf(buf, sizeof(buf),
           "Medians from %d stations: temp=%dºF, wind speed=%d knots", entries,
           (int)median_temp, (int)median_wind_speed);
  logger_.log(buf);
...
HTTPTask::HTTPTask(SplitflapTask &splitflap_task, DisplayTask &display_task,
    ...
    : Task("HTTP", 8192, 1, task_core), splitflap_task_(splitflap_task),
      display_task_(display_task), logger_(logger), wifi_client_() {}
#else
HTTPTask::HTTPTask(SplitflapTask &splitflap_task, Logger &logger,
                   const uint8_t task_core)
    : Task("HTTP", 8192, 1, task_core), splitflap_task_(splitflap_task),
      logger_(logger), wifi_client_() {}
#endif
...
void HTTPTask::connectWifi() {
  ...
  NetworkConfig::begin();
  char buf[256];
  logger_.log("Establishing Ethernet connection..");
#if ENABLE_DISPLAY
  display_task_.setMessage(1, String("Ethernet connecting..."));
#endif
#else
  NetworkConfig::begin(WIFI_SSID, WIFI_PASSWORD);
  
  // Disable WiFi sleep as it causes glitches on pin 39; see
  // https://github.com/espressif/arduino-esp32/issues/4903#issuecomment-793187707
  ...
  snprintf(buf, sizeof(buf), "Connected to network, IP: %s", NetworkConfig::localIP().c_str());
  logger_.log(buf);
...
void HTTPTask::run() {
  ...
  while (1) {
    ...
    if (http_last_request_time_ == 0 ||
        now - http_last_request_time_ > REQUEST_INTERVAL_MILLIS) {
      if (fetchData()) {
    ...
    if (update ||
        now - last_message_change_time_ > MESSAGE_CYCLE_INTERVAL_MILLIS) {
      ...
      if (messages_.size() > 0) {
        ...
        snprintf(buf, sizeof(buf), "Cycling to next message: %s",
                 message.c_str());
        logger_.log(buf);
